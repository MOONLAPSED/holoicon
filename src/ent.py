...
class MemoryCell:
    ...
    def entropy(self) -> float:
        """Calculate Shannon entropy for the stored value."""
        if not self.value:
            return 0.0
        # Calculate frequency of each byte
        freq = {byte: self.value.count(byte) for byte in set(self.value)}
        # Calculate probabilities
        probs = [count / len(self.value) for count in freq.values()]
...
class VirtualMemoryFS:
    ...
    Uses 16-bit addressing (0x0000 to 0xFFFF).
    """
...
"""
    ...
    def analyze_memory(self):
        """Analyze memory cells and print their entropy."""
        for address, cell in self._memory_cache.items():
            print(f"Address: 0x{address:04x}, Value: {cell.value.hex()}, Entropy: {cell.entropy():.2f}")
...
if __name__ == "__main__":
    ...
    vmem.write(0x1234, bytes([random.randint(0, 255)]))  # Random byte
